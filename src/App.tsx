...
function App() {
  ...
  const [previewMode, setPreviewMode] = createStore({
    mode: "code",
    enabled: false,
  });
  ...
  createEffect(async () => {
    ...
    setPreviewMode({
      enabled:
        savedFlags.find(
          (f: { id: string }) => f.id === "message-box-live-preview"
        )?.checked ?? false,
    });
  });
...
[s]${lipsync() ? "[f 4 10 65535 0 0]" : ""}${
  ...
  return (
    <PortraitConfigProvider>
      <div
        class="flex flex-col font-outfit"
                      ...
                      <div class="m-2">
                        ...
                        <div class="mt-4 flex items-center gap-2">
                          <input
                            ...
                            checked={lipsync()}
                            onChange={(e) =>
                              setLipsync(e.currentTarget.checked)
                            }
                          />
                          <label
                            for="lipsync"
                        ...
                        <div class="absolute top-4 right-4 w-96">
                          <div class="select-none">
                            ...
                            />
                            <Show when={previewMode.enabled}>
                              <Dropdown
                                options={PreviewModes}
                                value={previewMode.mode}
                                onChange={(value) =>
                                  setPreviewMode({ mode: value.toString() })
                                }
                                label="Preview mode"
                              />
                            </Show>
                          ...
                          <Show when={previewMode.mode === "live"}>
                            <div class="mt-2">
                              <Button
                                onClick={() => setConfig({ modalOpen: true })}
                                class="w-full"
                                variant="secondary"
                              >
                          ...
                          <PortraitConfig
                            isOpen={config.modalOpen}
                            onClose={() => setConfig({ modalOpen: false })}
                            onSave={() => {
                              setConfig({ modalOpen: false });
                            }}
                          />
                        </div>
                        ...
                        >
                          <div class="absolute bottom-4 left-4 w-[50rem] max-h-[208px] overflow-y-auto bg-secondary text-text p-1">
                            <p class="text-zinc-400 text-sm font-medium select-none">
                              Live preview
                            </p>
                            <Show
...
[s][bup 0 10 201 0 0][f 4 10 65535 0 0][vp 8 2 0 57157 65535 0][f 5 13 33 5 10]${string()}[f 1 3 65535][w][e]`} */}
        ...
        <Alert
          isOpen={compileModal()}
          onOk={handleCompile}
          onClose={() => setCompileModal(false)}
          title="Compile"
        >
          <p class="text-text">Do you want to compile the output file?</p>
        </Alert>
